...
def construct_dataset(sequences, labels, cuda, batch_size):
    ...
    return data_loader
# test_dataset, test_label, len_test = util_file.load_tsv_format_data('../data/lncRNA_test_data.tsv')
# test_dataset, test_label, len_test = util_file.load_tsv_format_data('../data/lncRNA_mus_test_data.tsv')
test_dataset, test_label, len_test = util_file.load_tsv_format_data('../data/mRNA_test_data.tsv')
# test_dataset, test_label, len_test = util_file.load_tsv_format_data('../data/mRNA_mus_test_data.tsv')
batch_size = 1
...
def plot_prc_curve(label_real, pred_prob):
    # 计算Precision和Recall
    precision, recall, _ = precision_recall_curve(label_real, pred_prob)
    average_precision = average_precision_score(label_real, pred_prob)

    # 绘制PRC曲线
    plt.figure()
    plt.plot(recall, precision, color='blue', lw=2, label='PRC curve (area = %0.2f)' % average_precision)
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.xlabel('Recall')
...
def save_data_to_file(roc_data, prc_data, roc_filename='bert_AUROC_data.txt', prc_filename='bert_prc_data.txt'):
    # Ensure the dimensions match for ROC data
    fpr, tpr, _ = roc_data
    roc_data_to_save = np.column_stack((fpr, tpr))

    # Save ROC data
    np.savetxt(roc_filename, roc_data_to_save, delimiter='\t', header='FPR\tTPR', comments='')

    # Ensure the dimensions match for PRC data
    recall, precision, _ = prc_data
    prc_data_to_save = np.column_stack((recall, precision))

    # Save PRC data
    np.savetxt(prc_filename, prc_data_to_save, delimiter='\t', header='Recall\tPrecision', comments='')
'''DNAbert2 model'''
class DNABERT2(nn.Module):
    def __init__(self):
        super(DNABERT2, self).__init__()
        # 加载预训练模型参数
        self.pretrainpath = '../pretrain/DNABERT2-pretrain'
    ...
    def forward(self, seqs):
        ...
        if cuda:
            _,representation = self.bert(input_ids.cuda(), token_type_ids.cuda(), attention_mask.cuda())
...
output_file = 'attention_scores_output.txt'
...
def predict():
    ...
    with torch.no_grad():
        ...
        for batch in loop:
            data, label = batch
            logits, representation = model(data)
        ...
        plot_prc_curve(label_real, pred_prob)
        # plot_confusion_matrix_with_metrics(label_real, label_pred)
        # save_data_to_file(ROC_data, PRC_data, 'mRNA_AUROC_data.txt', 'mRNA_prc_data.txt')
